...
A continuación, a modo de ejemplo de como complementar un dato primario con otro secundario, se hará un análisis que utilice tanto datos de la encuesta (como fuente primaria) como algunos datos del censo de 2010 (como fuente secundaria). El tema a tratar será hacinamiento.
...
Un punto metodológico interesante del concepto de hacinamiento, al menos antes de la decisión de considerar un punto de corte a partir del cual un hogar se considera hacinado, es que se puede considerar como un ejemplo de concepto cuantitativo [@klimovsky1998, pag. 252-257]. Entre otras características, podremos con el realizar legítimamente una serie de operaciones matemáticas (dividir, multiplicar, etc.) asumiendo que los valores de esas operaciones "representan" valores de los objetos medidos. En el caso específico del concepto de hacinamiento, que utiliza la operación de **dividir** al relacionar la cantidad de personas con la cantidad de ambientes, lo anterior es posible porque su medición empírica implica la medición previa de 2 variables con un nivel de medición de "razón". En este sentido, también se suele afirmar que hacinamiento es un ejemplo de una **variable compuesta** o **índice** ya que el hacinamiento, como variable, se "calcula" más que "medirse". Lo que se mide, a través de operaciones empíricas, son las variables que operan de insumo (p.e. cantidad de habitaciones, cantidad de personas).

Esta característica pueder servir para mostrar el siguiente problema: ¿Se puede calcular un índice de hacinamiento relacionando las cantidades *agregadas* de habitaciones y personas de un determinado territorio? ¿El resultado es el mismo si primero se calcula el hacinamiento para cada hogar y luego se calcula un promedio de esos valores?. Si hay diferencias entre ambas estrategias, ¿Cuál es el valor correcto?.

En primer lugar, que el resultado sea igual tanto con la estrategia agregada o la desagregada es una contingencia empírica. No hay nada necesario para que ambos valores sean iguales, aproximadamente similiares o muy diferentes. Dado lo anterior, el problema es pensar cual es la estrategia correcta. Aquí la clave está en cual es/son el/los referente/s del concepto que se agrega/n. ¿Son los hogares? ¿Son cada uno de los individuos que componen esos hogares? ¿En qué sentido se puede afirmar que un barrio se encuentra hacinado, o tiene "x" nivel de hacinamiento o, por lo menos, que se encuentra más o menos hacinado que otro barrio?. En principio, esto se debería resolver con un análisis conceptual inpeccionando las definiciones de los conceptos utilizados. Como se detalla en la nota de arriba sobre la historia del concepto de hacinamiento, el mismo refiere a los hogares, por lo que para su cálculo se debe tener, al mismo tiempo, la información de la cantidad de habitaciones de la vivienda y la cantidad de personas que componen el hogar.

A modo de ejemplo, a continuación se calcula la media de hacinamiento tanto de manera agregada como de manera desagregada.

```{r}
#| label: hacinamiento_desagregado
...
# Con custom summary no hace falta agregar especificar muchos campos

hacinamiento_desagregado = df_encuesta |>
  tbl_custom_summary(
    include = hacinamiento,
    stat_fns = ~ continuous_summary("hacinamiento"),
    statistic = ~"{mean}")

hacinamiento_desagregado
```

```{r}
#| label: hacinamiento_agregado
#| echo: false

hacinamiento_agregado_intermedio <- df_encuesta |>
select(viv_n_ambientes, hog_n_miembros_hogar) |>
summarise(viv_n_ambientes_mean = mean(viv_n_ambientes),
          hog_n_miembros_hogar_mean = mean(hog_n_miembros_hogar)) 

hacinamiento_agregado <- hacinamiento_agregado_intermedio |>
summarise("Hacinamiento agregado" = round(mean(hog_n_miembros_hogar_mean / viv_n_ambientes_mean), 1)) |>
 tbl_custom_summary(
    include = "Hacinamiento agregado",
    stat_fns = ~ continuous_summary("Hacinamiento agregado"),
    statistic = ~"{mean}")
...
hacinamiento_agregado

testing1 <- hacinamiento_agregado_intermedio[1]
testing2 <- hacinamiento_agregado_intermedio[2]
testing3 <- round(testing2/testing1, 2)
# testing1 <- as.numeric(inline_text(hacinamiento_agregado_intermedio, variable = viv_n_ambientes))
# testing2 <- as.numeric(inline_text(hacinamiento_agregado_intermedio, variable = hog_n_miembros_hogar))
# 
# testing3 <- round(testing2/testing1, 2)
```
...
Hay que agregar gráficas de las distribución del hacinamiento desagregado
